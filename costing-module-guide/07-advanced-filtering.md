...
> **Implementation location:** Within `src/app/admin/costing/page.tsx` (inline drawer, matching the purchasing page pattern)

---

## Overview

The costing module has two filtering layers, exactly like the purchasing page:
...
Full implementation matching the purchasing page drawer design:
...
|-------------|-------------------|-------|
| `sizeFilter` | `?size=M` | Exact match |
| `descriptionFilter` | `?description=Cotton+slab` | Exact match (from purchasing dropdown) |
| `minProfitFilter` | `?minProfit=20` | Range filter |
| `maxProfitFilter` | `?maxProfit=40` | Range filter |
| `minCostFilter` | `?minTotalCost=200` | Range filter |
| `maxCostFilter` | `?maxTotalCost=1000` | Range filter |
| `minSellingPriceFilter` | `?minSellingPrice=500` | Range filter |
| `maxSellingPriceFilter` | `?maxSellingPrice=2000` | Range filter |
...
useEffect(() => {
  ...
  minCostFilter,
  maxCostFilter,
  minSellingPriceFilter,
  maxSellingPriceFilter,
]);
```
...
- [x] Drawer uses `max-w-sm` (same as purchasing)
