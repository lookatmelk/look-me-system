...
> **Implementation location:** `src/components/orders/OrderDetailModal.tsx`

---

## Overview

When a user clicks a row in the orders table (or the Eye icon), a modal displays all the order details in a structured, read-only view. This follows the exact same pattern as `CostingDetailModal` in the costing module.
...
import { Hash, Package, Store, Calendar, FileText, TrendingUp, DollarSign } from 'lucide-react';
...
## Section 1: Revenue Summary Hero
...
## Section 3: Shop Allocations Breakdown
...
<div className="px-6 py-5 border-b border-slate-100">
  ...
  <div className="space-y-3">
    ...
    <ShopRow
      shopName="Shop 1"
      shopColor="blue"
      qty={record.shop1?.qty || 0}
      sizes={record.shop1?.sizes || []}
    ...
    <ShopRow
      shopName="Shop 2"
      shopColor="violet"
      qty={record.shop2?.qty || 0}
      sizes={record.shop2?.sizes || []}
      sellingPrice={record.sellingPrice}
    />
    ...
    <ShopRow
      shopName="Shop 3"
      shopColor="emerald"
      qty={record.shop3?.qty || 0}
      sizes={record.shop3?.sizes || []}
      sellingPrice={record.sellingPrice}
    />
...
function ShopRow({
  ...
  return (
    <div className={clsx('flex justify-between items-center px-4 py-3 rounded-lg border', colorMap[shopColor])}>
      <div>
        <div className="flex items-center gap-2">
          <Store className={clsx('h-3.5 w-3.5', textColor[shopColor])} />
          <span className="text-sm font-semibold text-slate-700">{shopName}</span>
        </div>
        {sizes.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-1.5">
            {sizes.map(size => (
              <span key={size} className="px-2 py-0.5 rounded-md text-[10px] font-semibold bg-white border border-slate-200 text-slate-600">
                {size}
              </span>
            ))}
          </div>
      ...
      <div className="text-right">
        <span className={clsx('text-lg font-black font-mono', qty > 0 ? textColor[shopColor] : 'text-slate-300')}>
...
2. **Size `"lg"`** matches the costing detail modal
3. **Gradient hero section** shows Design Total, Projected Revenue, Projected Profit
4. **Shop allocation breakdown** uses color-coded rows (blue, violet, emerald) with size chips and per-shop revenue
5. **Costing snapshot** shows the pricing that was locked at order creation time
6. **Status badge** uses the same color scheme as the table column
7. **Lucide icons** used: `Hash`, `Package`, `Store`, `Calendar`, `FileText`, `TrendingUp`, `DollarSign`

---

> **Next:** [09-relations-guide.md](./09-relations-guide.md)
